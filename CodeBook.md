...
## Generating "tidy.txt" from script "run_analysis.R"
The script "run_analysis.R"" was created to generate a tidy data set ("tidy.txt"") from text files obtained from https://d396qusza40orc.cloudfront.net/getdata%2Fprojectfiles%2FUCI%20HAR%20Dataset.zip
...
The file tidy.txt contains the average of each variable from X_train and X_test for each activity and each subject.  Thus the values are the average of two trials of each activity by each subject.  The variable names were maintained from the original dataset (https://d396qusza40orc.cloudfront.net/getdata%2Fprojectfiles%2FUCI%20HAR%20Dataset.zip) and as described below (see "Feature Selection") by the original authors.
...
## Feature Selection for "tidy.txt""
...
## Complete list of Features in "tidy.txt""
(all but study_subject and activity were taken from "features.txt".)
